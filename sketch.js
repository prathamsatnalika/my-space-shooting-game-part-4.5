...
function draw() {
  ...
  if(GAMESTATE === play_INSTRUCTION) {
    ...
    fill("red")
    text("THANK U FOR USING SPACE HARRIER, TO PLAY THIS GAME U NEED A PC, IF YOU WANT TO PLAY THIS GAME.",width/10.5,height-height/1.5);
    text("TO MOVE YOUR SPACESHIP, USE YOUR ARROW KEYS. YOUR MAIN MOTO IS TO DESTROY ALL THE SPACESHIP, ",width/10.5,height-height/1.7);
    text("HOW MUCH YOU CAN,BUT REMEMBER BE SAFE FROM ASTERIODS. AND DONT FORGET THAT ENEMY",width/10.5,height-height/1.9)
    text("SPACESHIP CAN ALSO SHOOT YOU. YOU CAN ALSO SHOOT BY PRESSING ENTER...",width/10.5,height-height/2.1);
    text("-THANK YOU",width/10.5,height-height/2.7);
